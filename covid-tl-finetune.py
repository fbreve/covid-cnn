...
def create_model(model_type, frozen_blocks):
    ...
    # mark frozen layers as not trainable
    if "EfficientNet" in MODEL_TYPE:
        for block in range(1,frozen_blocks+1):
            for layer in model.layers:
                if ('block'+str(block)) in layer.name:
                    layer.trainable = False
        for block in range(frozen_blocks+1,8):           
            for layer in model.layers:
                if ('block'+str(block)) in layer.name:
        ...
        for block in range(1,frozen_blocks+1):    
            for layer in model.layers:
                if ('block'+str(block)+'_') in layer.name:
                    layer.trainable = False
...
def train_test_model(train_df, test_df, model, preprocessing_function, image_size, random_state):
    ...
    train_df, validate_df = train_test_split(
        train_df, test_size=0.20, stratify=train_df.category, shuffle=True, random_state=random_state)
    ...
    predictions = model.predict(test_generator, steps=np.ceil(total_test/BATCH_SIZE))
...
# create filenames
log_filename = "covid-tl-ft-" + hostname + ".log"
csv_filename = "covid-tl-ft-res-" + hostname + ".csv"
...
with open(log_filename,"a+") as f_log:
    ...
    f_log.write("Maximum Frozen Blocks: %s\n" % MAX_FROZEN)
    f_log.write("Model: %s\n" % MODEL_TYPE)
    f_log.write("N Repeats: %s\n" % N_REPEATS)
    f_log.write("Batch Size: %s\n\n" % BATCH_SIZE)
         
for frozen_blocks in range(0,MAX_FROZEN+1):
    ...
    model, preprocessing_function, image_size = create_model(MODEL_TYPE, frozen_blocks)
    
    # save weights before training the model
    ini_weights = model.get_weights()        
    
    # record amount of frozen blocks in log file
    with open(log_filename,"a+") as f_log:
        f_log.write("Frozen Blocks: %s\n\n" % frozen_blocks)
    ...
    for rep in range(0,N_REPEATS):
        ...
        print("Frozen Blocks: %s Repetition: %i of %i" % (frozen_blocks, rep+1, N_REPEATS))
